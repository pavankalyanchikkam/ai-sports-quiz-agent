...
def compile_quiz_data(sport, difficulty):
    ...
    db_query = f"{sport} history cup world championships records rules"
    db_matches = query_historic_facts(sport=sport, query_text=db_query, n_results=5)
    db_context = "\n".join(db_matches)

    web_context = get_live_news_context(sport)
    
    # Structure context cleanly based on web result presence
    ...
    system_instruction = (
        ...
        "player statistics, or record holders. Avoid general knowledge that casual fans would know.\n\n"
        "STRICT CONTEXT USAGE RULE:\n"
        "All questions must test specific real-world sports facts: dates, scores, records, player names, or rules. "
        "Never create questions about what an article reports, what a website headline says, where to find scores, "
        "or what updates highlight. Do not formulate questions based on search metadata or website descriptions."
    )
    ...
    user_prompt = (
        ...
        "C. [Option C]\n"
        "D. [Option D]\n"
        "Correct Answer: [Single Letter, e.g., A. Answer Text]\n"
